...
crew agent register <agent-id> --provider <provider> --roles <csv> --capabilities <csv> [--a2a-url <url>] [--a2a-tenant <tenant>]
crew agent list
crew agent heartbeat <agent-id>
crew agent claim <run-id> <task-id> <agent-id> [--lease-seconds 300]
crew agent release <run-id> <task-id> <agent-id>
...
Agents register a stable identity and capabilities, then can discover peers, claim/release tasks using leases, exchange messages, and inspect their inbox. Claims are accepted only while the run is working, and the registered agent must match the task role and include every capability requested by that task. These operations do not call the engine progression loop.

Messages are journaled to the durable local mailbox before optional direct delivery is attempted. When a recipient registers an A2A endpoint, direct A2A additionally requires the sender's registered `network` capability and the run snapshot's `permissions.network = "allow"`; `ask` and `deny` stay mailbox-only. Eligible direct delivery uses A2A 1.0 JSON-RPC `SendMessage`, and a registered interface tenant is echoed on the request. A delivery receipt overlays the durable mailbox record after the direct attempt, so a slow or failed network cannot delay creation of the local audit record. If direct delivery is unavailable or fails, mailbox delivery remains functional. A2A authorization values can be supplied by the TypeScript API as environment-variable references; secret values are never stored in registration files.

Force-releasing a lease is intentionally unavailable through the ordinary peer CLI. Trusted TypeScript callers can use `AgentMesh.releaseTask(..., true)` for recovery tooling.
...
│   ├── communication/.delivery/<message-id>.json
│   ├── artifacts/
│   ├── context/
│   └── tasks/
├── history/<run-id>/
