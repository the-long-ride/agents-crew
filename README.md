...
- Host plugin installer for Codex, Antigravity, Claude Code, OpenCode, and GitHub Copilot
...
agents-crew help plugin     # host plugin install/uninstall/doctor help
...
`agents-crew plugin` generates workspace-local files that let Codex, Antigravity, Claude Code, OpenCode, and GitHub Copilot call the same core CLI and `.agents-crew/` state model.
...
Generated files contain no secrets, existing files are skipped unless `--force` is used, and uninstall removes only files marked as generated by `agents-crew`. See [docs/plugins.md](docs/plugins.md) for host paths, examples, and failure handling.
