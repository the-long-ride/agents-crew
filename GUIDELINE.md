...
src/
  ...
  plugins/       host plugin contracts, manifests, installer, and hook templates
...
docs/            workflow, adapter, plugin reference, tutorial
...
- **Plugin files need generated markers.** Anything removable by `agents-crew plugin uninstall` must contain `generated by agents-crew plugin installer`; never remove unmarked user files.
...
- `tests/docs.test.mjs` requires every doc under `docs/workflows/`, `docs/adapters/`, `docs/plugins.md`, and `docs/tutorial.md` to mention **setup**, **failure**, and **example**. Update docs alongside behavior changes.
...
8. For plugin changes, run plugin CLI tests and verify install, dry-run, force, uninstall skip, and doctor behavior.
