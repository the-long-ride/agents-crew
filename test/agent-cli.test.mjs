...
async function workspaceWithTask() {
  ...
  const config = structuredClone(api.starterConfig());
...
test('agent capabilities are available without initializing or advancing a run', async () => {
  ...
  assert.deepEqual(result.operations, ['register', 'list', 'heartbeat', 'claim', 'release', 'complete', 'send', 'inbox', 'capabilities']);
});
...
test('compiled CLI exposes machine-readable agent capabilities', async () => {
  ...
  assert.ok(payload.operations.includes('complete'));
