...
import type { AgentMessageKind, Capability, Role, WorkerResult } from '../domain/types.js';
import { AgentMesh } from '../orchestration/agent-mesh.js';

export const agentOperations = ['register', 'list', 'heartbeat', 'claim', 'release', 'complete', 'send', 'inbox', 'capabilities'] as const;
...
export function agentCapabilities(): { protocol: string; a2a: string; operations: string[]; transport: { direct: string; fallback: string } } {
  return {
...
export async function dispatchAgentCommand(parsed: ParsedArgs): Promise<unknown> {
  ...
  if (subcommand === 'register') {
    ...
    const interfaces = a2aUrl ? [{
      kind: 'a2a' as const,
      url: a2aUrl,
      protocol_binding: 'JSONRPC' as const,
      protocol_version: optionalText(args.a2a_version) ?? '1.0',
      tenant: optionalText(args.a2a_tenant),
    }] : [];
    return {
  ...
  if (subcommand === 'release') {
    if (args.force === true) throw new Error('force release is reserved for trusted callers and is not available through the agent CLI');
    const runId = text(values[0], 'run id');
    const taskId = text(values[1], 'task id');
    const agentId = text(values[2], 'agent id');
    await mesh.releaseTask(runId, taskId, agentId);
    return { run_id: runId, task_id: taskId, agent_id: agentId, released: true };
  }
  if (subcommand === 'complete') {
    const resultPath = resolve(text(args.result, 'result path'));
    const result = JSON.parse(await readFile(resultPath, 'utf8')) as WorkerResult;
    return {
      task: await mesh.completeTask(text(values[0], 'run id'), text(values[1], 'task id'), text(values[2], 'agent id'), result, resultPath),
    };
  }
  if (subcommand === 'send') {
    return {
