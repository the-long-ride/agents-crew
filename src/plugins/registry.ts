...
export const commands = [
  ['agents-crew', 'Orchestrate one complete user request through Agents Crew.'],
...
function orchestratorInstructions(host: Host): string {
  return `Treat \`$ARGUMENTS\` as the complete user goal. Run \`agents-crew orchestrate --host ${host} --goal "$ARGUMENTS" --json\`. Do not ask the user to initialize Agents Crew, name a crew, choose roles, or operate lifecycle commands. Process returned \`actions\` until the orchestrator reaches completed, awaits a real approval, or reports a genuine blocker. For a \`dispatch_native\` action, execute only the supplied task, workspace, context file, capabilities, and output schema; write normalized worker-result JSON to a temporary result file; then submit it with \`crew manager submit --run <run_id> --action <action_id> --result <result-file> --json\`. For a \`review\` action, inspect the durable run/task context, write manager-decision JSON matching the supplied schema, and submit it with the same \`crew manager submit\` command. When status is \`awaiting_approval\`, read the approval id, operation, and reason from \`pending_approvals\` and present only that decision to the user. If approved, run \`crew approve <approval_id> --run <run_id> --json\` and resume the original orchestrate command. If rejected, run \`crew reject <approval_id> --run <run_id> --json\`, report the blocked result, and stop; do not automatically start or resume another run. After each submitted dispatch/review action or approved approval, invoke the original \`agents-crew orchestrate --host ${host} --goal "$ARGUMENTS" --json\` again so the matching run resumes automatically. On completion, summarize returned \`evidence\` and \`verification\`. Stop only on terminal completion, explicit pause/cancel, unresolved blocker, rejected approval, or required user approval. Routine peer handoffs may use \`crew agent\` and AgentMesh without routing messages through the manager. Low-level manager, agent, and lifecycle commands are advanced/debug controls; never invent action IDs, bypass policy, or claim completion before the orchestrator returns completed. Host: ${host}.\n`;
}
export function commandContent(host: Host, name: string, description: string): string {
  ...
  if (host === 'claude-code') front = `---\ndescription: ${description}\n---\n\n`;
  if (host === 'antigravity') front = `---\nname: ${name}\ndescription: ${description}\n---\n\n`;
  if (name === 'agents-crew') return `${front}${orchestratorInstructions(host)}`;
