...
import { createRun, createTask, hasIndependentReview, nextBatch, taskGraph, taskWrites, verifyCompletion, verifyTaskResult } from '../domain/core.js';
import { decidePolicy, type PolicyOperation } from '../domain/policy.js';
...
export async function persistRun(workspace: string, run: Run): Promise<void> {
  ...
  const terminal = ['completed', 'cancelled', 'failed', 'blocked'].includes(run.status);
  if (terminal) {
